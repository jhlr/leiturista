...
| **Sinais de coerência do pipeline `leiturista` batem com a semântica da nota, sem rótulo?** | **Testado empiricamente, ampliado para 10 grupos** com `scripts/distribuidora_pipeline_eval.py`. Achado: o sinal "leitura não detectada" discrimina seletivamente — sobe em notas de semântica visual/física (T111 57,5%, L101 72,5%, M141 52,5% vs. NA 37,5%) mas fica **abaixo** do baseline em notas administrativas (T181 25,0%), evidência de que não é ruído genérico. **Bug real encontrado e corrigido** em `src/leiturista/inference.py`: a checagem de nitidez (Laplaciano) só rodava quando havia leitura detectada, mascarando o sinal na maioria dos casos — corrigido para avaliar sempre, expondo o valor contínuo em vez de só um booleano. **Validação cruzada negativa** no UFPR-AMR (dataset com gabarito real): nitidez isolada não prediz acerto/erro do OCR (r=-0,002, AUC≈0,43) — reforça que não dá pra confiar só nesse sinal. **Latência formal medida**: média 4,43s/imagem, mediana 1,4s, p90 13,2s (n=382, CPU). Resultado completo em `docs/sbti_artigo/artigo_sbti2026.docx` §4.3/4.3.1/4.4; detalhe com dado real do cliente em `data/distribuidora_campo/pipeline_eval.json` (gitignored, não commitar). |
